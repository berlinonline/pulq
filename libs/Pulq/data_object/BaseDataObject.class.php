...
abstract class BaseDataObject implements IDataObject
{
    ...
    {
        $scopeName = is_null($scopeName) ? static::SCOPE_FULL : $scopeName;
    ...
    protected function getArrayScope($scopeName)
    {
        ...
        if ($scopeName === static::SCOPE_FULL)
        {
        ...
        {
            $scopeName = $this->getDefaultArrayScope();
            if (!array_key_exists($scopeName, $scopes))
            {
                throw new ArrayScopeNotFoundException($scopeName);
            }
        }
    ...
    public static function toArrays(PulqDataObjectSet $objects, $scope = null)
    {
        $arrays = array();

        foreach($objects as $obj)
        {
            $arrays[] = $obj->toArray($scope);
        }
