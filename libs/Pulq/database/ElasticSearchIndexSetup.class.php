...
class ElasticSearchIndexSetup implements IDatabaseSetup
{
    ...
    public function setup($tearDownFirst = FALSE)
    {
        ...
        $rivers = $this->database->getParameter('rivers', array());
        if(!is_array($rivers))
        {
            $rivers = array();
        }

        foreach ($rivers as $name => $params)
        {
    ...
    protected function createRiver($name, array $riverParams)
    {
        ...
        $scriptFilePath = $setupDir . $riverSettings['couchdb']['script'];
        if (file_exists($scriptFilePath))
        {
            ...
            $riverSettings['couchdb']['script'] = shell_exec($uglifyPath . ' -nm -nc ' . $scriptFilePath);
        }
